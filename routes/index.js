...
router.get('/', function(req, res, next) {
  ...
  models['UserInfo'].getFirstValues(req, function (results) {
    let userDataObj = results ? results.dataValues : {};
...
router.get('/accept', function(req, res, next) {
  ...
  req.where = {'shop_name':shop};
  models['UserInfo'].getAccessToken(req, function (results) {
    let newObj = Object.assign({}, results.dataValues);
    newObj.id = newObj.charge_id;
    newObj.status = "accepted";
    newObj.billing_on = null;
    delete newObj.test;
    delete newObj.trial_days;
    let new_request = {
      "recurring_application_charge": newObj
    };
    // let new_request = {
    //   "recurring_application_charge": {
    //     // "id":12671320152,
    ...
    //   }
    // };
    // console.log('resultsresults', new_request);return;
